...
def shift_timestamps(orders):
    ...
    for order in orders:
        if 'ordered_at' in order: order['ordered_at'] = shift_date_str(order['ordered_at'])
        if 'estimated_delivery' in order: order['estimated_delivery'] = shift_date_str(order['estimated_delivery'])
        if 'delivered_at' in order: order['delivered_at'] = shift_date_str(order['delivered_at'])
        if 'return_deadline' in order: order['return_deadline'] = shift_date_str(order['return_deadline'])
        if 'shipment' in order:
            if 'estimated_delivery' in order['shipment']: order['shipment']['estimated_delivery'] = shift_date_str(order['shipment']['estimated_delivery'])
            if 'delivered_at' in order['shipment']: order['shipment']['delivered_at'] = shift_date_str(order['shipment']['delivered_at'])
    return orders
