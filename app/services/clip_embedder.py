...
def load_model():
    ...
    try:
        processor = AutoProcessor.from_pretrained(model_name, token=hf_token, local_files_only=True)
        model = AutoModel.from_pretrained(model_name, token=hf_token, local_files_only=True)
    except Exception:
        import logging
        logging.getLogger(__name__).info("Model not found in cache, downloading from HuggingFace...")
