...
2. **Global Redis Semaphore (Layer 2 - The LLM Funnel):** To protect the Groq AI Free Tier (30 RPM limit), the entire LangGraph execution is wrapped in a distributed Redis Semaphore. No matter how many Uvicorn workers are running in the Backend Cluster, exactly 10 requests are allowed to execute simultaneously. Any excess traffic seamlessly queues in memory within the workers and is processed as tokens become available, maximizing throughput without ever triggering API blocks.
